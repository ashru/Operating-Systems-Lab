...
void implement_cp(char *f1,char *f2)/*implementatio of command cp*/
{
	...
	if( access( f1, F_OK ) == -1 ) //check for permissions
	{
    ...
    else if( access( f2, W_OK ) == -1  &&  access( f2, F_OK ) != -1 )//check for permissions
    {
    ...
    else
    {
        ...
        if(difftime(stat1.st_mtime,stat2.st_mtime)<0)// check for difference in last modified time
            {
        ...
        int ofd = open(f2, O_CREAT | O_WRONLY |O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH | S_IXOTH);//permission flags
...
int ifd,ofd;int flaginp=0,flagop=0,flagpipe=0;int pip_in[100];//max 100 pipes allowed
	...
	if(flagpipe==0)//if there are no pipes
	{
		...
		{
			umask(0);
			if (setsid()==-1) //set as independent process
			{
	...
	else
	{
        		...
        		id = fork();
	  			...
	  			else if(id == 0)
	  			{
            		...
            		if(pipe_n != 0)
            		{
	   				...
	   				close(pipes[pipe_n][0]);//close read end of pipes
	    			...
	    			if( execvp(tokenlist[0], tokenlist)==-1)//call exec
	    			{
	  			...
	  			{
	  				close(pipes[pipe_n][1]);// close write end of all pipes
		...
		/*code for the last executble in the pipe sequence*/
		id=fork();
		if(id==0)
		{
...
void parse(char * input,char *tokenlist[])//parse string input.assume space are present before <,>,|
{
